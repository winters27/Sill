...
fn contact_sheet() {
    ...
    let from: i32 = std::env::var("FROM").unwrap_or_else(|_| "0".into()).parse().unwrap();
    let count: i32 = std::env::var("COUNT").unwrap_or_else(|_| "64".into()).parse().unwrap();
    ...
    for step in 0..count {
        ...
        let Some(b64) = uri.split(',').next_back() else { continue };
        ...
        if info.color_type != png::ColorType::Rgba { continue }
    ...
    encoder.write_header().unwrap().write_image_data(&canvas).unwrap();
