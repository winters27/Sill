...
| P2.7 | Terminal execution, capability gated | **Done as a layer.** `shell.rs` runs a command with three bounds: output capped and the pipes still drained, a deadline, and cancellation, all three killing the whole process tree through a job object. `ShellExecution` is its own capability, so the model raises a card and extensions cannot reach it. A surface of its own, with output on screen and a Stop button, is not built: today it is reached by running a script |
| P2.8 | Scripting | **Done, less arguments.** Raycast script headers are read as written, in PowerShell, cmd, bash, Python or a bare executable. Found by search, run from the launcher, output shown per the header's mode. **A script declaring a required argument says so rather than running**: arguments reach a quicklink through the launcher's argument mode, and wiring scripts into that path is the open half |
