...
| P1.11 | Extension install path | **Done.** 2026-09-05: Sill ships its own Node runtime (`scripts/fetch-node.mjs` pins one LTS build by version and SHA-256, bundled as `node/node.exe`, preferred over any Node on the PATH), and the installer adds one inbound firewall rule for that runtime on private and domain networks so a granted extension can hear the network; the uninstaller removes it (`leavings.rs`). Point at a folder and its commands are built and listed, or browse the store and install from there. esbuild ships beside the host; the build is Rust, not a repository script |
...
| P3.3 | Extension store | **Done**, separately. 2026-09-05: an extension command ranks above the application of the same name within one match class (`from_an_extension` in the registry sort), and an extension's name is matched as a name for every command it provides rather than as a category, which was `Elsewhere` (five tests, two of them sabotage-checked). Grid tiles draw built-in icons and colour swatches (`gridContentOf`). A relative path into an extension's own `assets`, in any view, is read through Rust by session (`extension_asset`, `asset_path` refusing a name that climbs) and cached per window (`$lib/exthost/assets`). The store is a list first and a page second: opening a row (Enter or click) shows the listing on its own screen, with the screenshots Raycast shows for it (`store_gallery`, fetched from the listing page for that one extension when it is opened and kept with the catalogue), its commands, version and the one key that installs it; Escape goes back to the list. A row carries its category mark, install count, Installed/Update pill and author picture on the right; the chip strip is replaced by one filter control beside the search field (`StoreFilter`, in the `accessory` slot of `SearchRow`) offering the scope and every category with the mark Rust chose for it (`store::category_mark`); the store draws "Installed" and "Update" as pills, the detail pane shows the key and the verb, and the hint strip uses keycaps at the meta size |
