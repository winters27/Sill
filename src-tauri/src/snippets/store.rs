...
pub fn match_keyword<'a>(snippets: &'a [Snippet], typed: &str) -> Option<&'a Snippet> {
    snippets
        .iter()
        .filter(|snippet| {
        ...
        .max_by_key(|snippet| snippet.keyword.trim().chars().count())
}
...
mod tests {
    ...
    fn a_keyword_matches_at_the_end_of_what_was_typed() {
        ...
        assert!(match_keyword(&snippets, "hello there ;sig").is_some());
        assert!(match_keyword(&snippets, ";sig").is_some());
        assert!(match_keyword(&snippets, ";sig and more").is_none());
    }
    ...
    fn turning_off_whole_word_lets_it_fire_anywhere() {
        ...
        assert!(match_keyword(&snippets, "resig").is_some());
    }
    ...
    fn a_punctuation_prefixed_keyword_works_either_way() {
        ...
        let snippets = vec![snippet("a", ";sig")];
        assert!(match_keyword(&snippets, "hello;sig").is_some());
        assert!(match_keyword(&snippets, "hello ;sig").is_some());
    }
    ...
    fn a_snippet_with_no_keyword_never_fires() {
        ...
        assert!(match_keyword(&snippets, "anything at all").is_none());
        assert!(match_keyword(&snippets, "").is_none());
    }
