...
/// Asks a process to close, the way clicking its X does.
///
/// **Asked, not killed.** A program told to close saves what it was doing;
/// one that is terminated does not, and a launcher that loses somebody's
/// unsaved work to a mis-pressed Enter is a launcher they stop trusting. The
/// harsher one exists too and says what it is.
#[cfg(windows)]
pub fn ask_to_close(pid: u32) -> Result<(), String> {
    ...
    for window in windows_of(pid) {
        // SAFETY: the window came from an enumeration a moment ago; a handle
        // that has since gone makes this fail rather than misbehave.
        let sent = unsafe { PostMessageW(Some(window), WM_CLOSE, WPARAM(0), LPARAM(0)) };
        asked |= sent.is_ok();
    }
    ...
    if asked {
        ...
        // Nothing to close politely. Said rather than escalated: quitting a
        // windowless service is not what somebody meant by "close this".
        Err("That has no window to close. Use Force Quit if you mean to end it.".to_string())
    }
}

/// Ends a process outright.
#[cfg(windows)]
pub fn force_quit(pid: u32) -> Result<(), String> {
    use windows::Win32::Foundation::CloseHandle;
    use windows::Win32::System::Threading::{OpenProcess, PROCESS_TERMINATE};

    // SAFETY: the handle is closed on both paths out.
    let handle = unsafe { OpenProcess(PROCESS_TERMINATE, false, pid) }
        .map_err(|err| format!("that process will not be ended: {err}"))?;

    let ended = unsafe { windows::Win32::System::Threading::TerminateProcess(handle, 1) };
    unsafe {
        let _ = CloseHandle(handle);
    }

    ended.map_err(|err| format!("that process will not be ended: {err}"))
}

/// Every visible window belonging to one process.
#[cfg(windows)]
fn windows_of(pid: u32) -> Vec<windows::Win32::Foundation::HWND> {
    ...
    unsafe extern "system" fn visit(window: HWND, into: LPARAM) -> BOOL {
        // SAFETY: `into` addresses the struct below, which outlives the call.
        let looking = unsafe { &mut *(into.0 as *mut Looking) };

        if unsafe { IsWindowVisible(window) }.as_bool() {
            let mut pid = 0u32;
            unsafe { GetWindowThreadProcessId(window, Some(&mut pid)) };
            if pid == looking.pid {
                looking.found.push(window);
            }
        }
    ...
    let mut looking = Looking { pid, found: Vec::new() };

    // SAFETY: as above.
    unsafe {
        let _ = EnumWindows(Some(visit), LPARAM(&mut looking as *mut _ as isize));
    }
...
#[cfg(not(windows))]
pub fn ask_to_close(_pid: u32) -> Result<(), String> {
    Err("only on Windows".to_string())
}

#[cfg(not(windows))]
pub fn force_quit(_pid: u32) -> Result<(), String> {
    Err("only on Windows".to_string())
}
