...
| P3.3 | Extension store | **Done**, separately. 2026-09-05: an extension command ranks above the application of the same name within one match class (`from_an_extension` in the registry sort, three tests); the store draws "Installed" and "Update" as pills, the detail pane shows the key and the verb, and the hint strip uses keycaps at the meta size |
